...
In Ruby, to express "nothing", `nil` is used. When `nil` is evaluated, it's treated as `false`. But while both `nil` and `false` are treated the same when evaluated, they are not equivalent.
...
Ruby considers everything to be truthy other than `false` and `nil`. While
