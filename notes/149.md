...
A **public method** is accessible to anyone who knows either the class name or the object's name. Public methods comprise a class's *interface*, or how other classes and objects can interact with the class and its objects.

Both **private methods** and **protected methods** are only accessible from within other methods of the class.
...
2. Sandwich code — when a method implementor wants to allow a method caller to do something before and after an action but doesn't care what that action is. The code in the block becomes that action.  
