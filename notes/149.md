...
OOP addresses this by using containers to bundle related attributes (or data) and behavior (or instructions). The container is called an **object** and the **class** is an outline of the attributes and behaviors available to the object.

Classes and Objects
-------------------

In Ruby, everything is an object. Strings, Arrays, Hashes, and Integers are a few of the many objects built-in to the Ruby language. Each type of object has different attributes and behaviors available to it, all of which is outlined in their respective classes.

When defining a class, we typically focus on two things: *attributes* and *behaviors*. Attributes track information for individual objects. Behaviors are what objects are capable of doing.
...
Let's modify the class by adding an `initialize` method. When defined, the `initialize` method is called each time an object is instantiated.  We refer to the initialize method as a *constructor*, because it gets triggered whenever we create a new object
...
Instance variables (`@`) track information about an object's state. The `Dog` class below has a `name` variable. And there are two instance methods that allow an object to assign (`set_name`) and retrieve (`get_name`) the name of a `Dog` object.  
...
`my_dog` and `your_dog` are different objects and may contain different states, such as the value of the `name` variable. Since they are both instances of the `Dog` class, the same behaviors (or methods) are available to both.
...
The `attr_acessor` method takes a symbol as an argument, which it uses to name the `getter` and `setter` methods. To create just a `getter` method, use `attr_reader` instead, and to create just a `setter` method, use `attr_writer`.
...
The problem is that Ruby thinks we're trying to create a new local variable named `owner`. In this situation, our expectation of syntactical sugar is coming into conflict with standard Ruby syntax. To clarify that we actually want to call the setter method, we employ the use of the `self` keyword.
...
Calling `self` from an instance method returns the calling object, in this case, `my_dog`. In this example, `self.owner` is the essentially the same as `my_dog.owner`

When `self` is prepended to a method definition, it is defining a **class method**. Class methods are methods we can call on the class itself, without having to instantiate any objects. Class methods are for functionality that doesn't pertain to individual objects.
...
When `self` is called inside a class but outside an instance method, it refers to the class itself. `def self.method` is essentially the same as `def Dog.method`.
...
It's important to remember that every custom class inherently subclasses from the `Object` class and therefore has access to its methods.

The to_s Method
---------------

Every Ruby object (and custom class) subclasses from the `Object` class and therefore has access to its methods.
...
It's generally recommended to **not** override these methods. One exception is the `to_s` method.
...
A common way of using `super` is with `initialize`. In the example below, the `name` argument is passed to the `initialize` method of the superclass, which assigns its value to the `@name` instance variable. The `@name` instance variable belongs to the `Dog` class and is accessible through the getter method.
...
If `super` is used with no arguments, it will automatically forward the arguments that were passed to the method from which it was called. In the example below, both the `name` and `color` variables are assigned to "Brown".
...
Like a class, a module is another way to group related behaviors. Unlike a class, you cannot instantiate an object nor can you subclass from a module. Modules are primarily used to add ("mix in") functionality to an existing class.  To use a module's functionality,  the `include` keyword is placed in the existing class definition. In the example below, `Dog` and `Human` have been given access to the `speak` method.
...
Modules can also act as a namespace, allowing you to group related classes together. This makes it easier to recognize related classes in our code and reduces the chance that a class could conflict with another similarly named class.

Modules can also act as a container for methods, called module methods. This is useful for methods that seem out of place in your code.

Inheritance vs. Modules
------------------------

While you can inherit from only one class, you can mix in any number of modules. Mix ins are Ruby's way of somewhat supporting multiple inheritance.

Inheritance is generally used for an "is a" relationship , while modules are used for a "has a" relationship. For example, a dog *is a* animal and a dog *has a* ability to speak.  

Method Lookup Path
------------------

Ruby has a distinct path it follows to find a method that has been called. In the example below, after first looking in the calling object's class, the program next looks within the mixed in modules, and lastly within the object's superclass. When handling multiple modules, Ruby actually looks at the last included module first and works up.
...
A variable can hold any kind of object, even that of a custom class. In the example below, a `Dog` object, `fitz`, is assigned to `@pet`. So when we call `me.pet`, it returns a `Dog` object.
...
When we test equality of two String objects using `String#==`, we are actually comparing values within the two objects, not the objects themselves. To check if two String objects are the same, we can use the `Object#equal?` method.
...
Ruby aims to be concise and succinct. To meet this aim, the language contains a lot of "syntactical sugar", or special syntax that creates more readable code while hiding what's actually going on underneath. For example, when calling the method `String#==`, we call it with a special syntax that reads more naturally (eg. str1 == str2) than the more method-like option (eg. str1.==(str2)). A consequence of Ruby's syntactical sugar is that it's difficult to know if an operator is actually an operator or simply a method disguised as a "fake operator".
...
Let's examine the `[]` and `[]=` methods using the two equivalent groups of code below. Note the second example takes advantage of Ruby's special syntax and reads more naturally.
...
Class variables start with `@@` and are scoped at the class level. All objects share one copy of a class variable. Class methods can access class variables regardless of where they're initialized.
...
Note that it's possible for a class variable to adversely affect the value of the class variable in a superclass and therefore its other subclasses. For this reason, class variables are generally avoided, especially when working with inheritance.

Constant Scope
-----------------------

When creating classes there may also be certain variables that you never want to change. A constant variables (or simply a **constant**) begins with an uppercase letter and has lexical scope. Unlike class and instance variables, constants from completely separate classes are accessible with the appropriate syntax.
...
1. Give *method caller* more power and flexibility when invoking your method.
...
2. Allow the *method caller* to do something before and after any specified action; the block's code being that action. There are many useful use cases for "sandwich code". Timing, logging, notification systems are all examples where before/after actions are important.
...
A `lambda` is a special type of `Proc` object. There are a couple key differences though. The first is arity; a `lambda` enforces the number of arguments, while a `Proc` does not. The second difference is how they each treat the `return` keyword.
