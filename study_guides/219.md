...
In JavaScript, every function creates a new scope. In addition (and different than Ruby), a function also has access to any variables defined in surrounding scopes.

In addition, a function has access to any variables that are available where the function was defined, but not necessarily where the function is called. In other words, every JavaScript function is actually a **closure**, in that it has access to its function definition and also the environment in which that function was defined. The environment consists of any local variables that were available when the function was defined.
