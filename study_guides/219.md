...
If a function does not contain an explicit `return` statement, the function implicitly returns `undefined`.
...
To clearly communicate whether your function has side effects, there are a few practices to follow:

1) Convert functions to pure functions whenever possible to eliminate side effects and possible bugs.
...
2) Pure functions require the programmer to assign the result of a function call to some variable. If your function has side effects, don't assign the result this way.
