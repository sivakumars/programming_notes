...
Arrays are the basic collection type used in JavaScript, and the hold values that are indexed by a non-negative integer. But JavaScript Arrays are really Objects.
...
Objects are containers for two things: data and behavior. The data consist of named items and their corresponding values; in JavaScript, we call them the properties of an object.

When functions are included as part of an object, we call them **methods**. Both data properties and methods can be accessed using dot-notation.
...
A property name for an object can be any valid string, and a property value can be any valid expression. If a property name does not contain spaces, the quotes can be omitted.
...
Object properties can be accessed in two ways. If a property name is a valid identifier, "dot notation" may be used. Otherwise, "bracket notation" must be used.  
...
The same rules apply for adding properties to a function. And you can use the keyword `delete` to delete properties from an object.
