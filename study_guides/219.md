...
console.log(country1);      // ReferenceError: country1 is not defined (country1 is local to the function)
}
```
...
You can also assign a variable to a named function, however, the function name is only accessible within the function. Though most function expressions use anonymous functions, named function expressions can be useful for debugging.
