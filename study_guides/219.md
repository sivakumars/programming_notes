...
In JavaScript, variables are declared before they are used with the `var` keyword. After a variable is declared, you can use the `=` operator to assign a value to it. Any variable that is declared but not assigned will be initialized to the value `undefined`.
...
JavaScript is a *loosely typed* or *dynamically typed* language, meaning you don't have to declare the type of variable ahead of time. In addition, you reassign a variable to a different data type without error.

Primitive Data Types
-------------------

A **primitive** is a data type that is not an object and has no methods.  JavaScript (ES5) contains five primitive data types: `number`, `boolean`, `string`, `null`, and `undefined`.

All primitives are *immutable* (cannot be changed in place). Therefore, to change the value of a variable, you must reassign it; no function, method, or operation will modify (or mutate) it for you.

All other data types, including arrays and functions, are JavaScript Objects. Objects are mutable, which means you can modify their contents in place.
...
A **coercion** or **conversion** is an operation that attempts to convert a variable's value from one data type to another. Since primitives are immutable, JS doesn't actually "convert" the values but instead returns a new value of the proper type.

Although there are two primary methods used to convert strings to numbers, the `parseInt` method with the optional radix specified is preferred.
...
A number (or a boolean) can be converted to a string using either a String object constructor function or by calling the `toString` method on an object. The constructor method is preferred because it doesn't thrown an error when passed `null`.
...
When evaluating expressions of different types, JavaScript will often attempt to **implicitly** (or automatically) convert one of the types to match the other. This JavaScript feature seems convenient but results in unexpected bugs. Therefore, as a rule, implicit type conversions should be avoided.
...
JavaScript is said to have "lexical scope". This means that the code within one scope can access any variables in the same scope or any surrounding scope. When JavaScript tries to find a variable, it searches the current scope first before proceeding to surrounding scopes. When it finds a match variable, it returns its value. This means that variables in an inner scope can *shadow*, or hide, a variable with the same name in an outer scope.
...
When assigning a variable to a primitive value, JavaScript creates a new reference in memory and points the variable to that primitive reference. If you were to assign a second variable to the value of the first variable, JavaScript creates a new reference which contains a copy of the value of the first variable's reference. Lastly, if you reassign the first variable to a new value, JavaScript will proceed to create a new reference again and point the first variable to that reference. The second variable will remain unaffected.
...
This demonstrates that JavaScript is pass-by-value when it comes to dealing with primitive data types.

Where JavaScript differs from Ruby is in the mutability of primitives. In Ruby, a string is mutable. In JavaScript, it is not.

### Objects

When it comes to objects, the behavior is similar in one regard.
...
Here, we assigned a first variable to an object and then a second variable to that same object.

Since we reassigned the first variable to a new object, JavaScript created a new reference and pointed the first variable to it. The second variable was unaffected again.

Due to this behavior, from a Computer Science theory perspective, JavaScript is purely pass-by-value for all data types (primitives and objects).

In practice, however, there is an important distinction.
...
In this example, we reassigned a property of an object to a new value. And that reassignment **did** affect the second variable. This is because when JavaScript assigns the second variable, it doesn't create a copy (as it does with primitives); instead it creates a reference that points to the same properties as the first object.
