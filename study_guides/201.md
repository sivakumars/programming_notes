...
The `position` CSS property identifies how an element is positioned on a page and also whether or not it will appear within the *normal flow*. It's used in conjunction with the box offset properties — `top`, `right`, `bottom`, and `left` — which determine how much and from which directions an element will be shifted relative to its containing block.

By default every element has a `position` value of `static`, which means that it exists in the normal flow of a document and does not accept any box offset properties. In order to accept these box offset properties, the `position` property must be given a value of `relative`, `absolute`, or `fixed`.

#### Relative Positioning

When `position: relative` is applied, the box is shifted from its original position. The original position of the box remains in the normal flow and therefore surrounding elements will behave as if the box remains in its non-offset position, regardless of where it's actually positioned.

#### Absolute Positioning

When `position: absolute` is applied, the box is shifted in relation to its closest ancestor element with a `position` value of `relative`. If none of its ancestors are relatively positioned, then it is positioned in relation to the `<body>` element. With absolute positioning, the element is removed from the *normal flow* and therefore other elements are able to occupy the space left by the element's original position.
...
When elements are positioned with the `position` property, it's possible that they'll be stacked on top of one another, potentially overlapping each other's content. To change the order of how overlapping elements are stacked, the `z-index` property is available.
...
To group multiple options and ensure only one is selected, each `type="radio"` element in a group must have the same value for the `name` attribute. And the value of the `value` attribute determines the data sent to the server.
...
Checkboxes are configured similarly except its functionality allows users to select any number of options.
