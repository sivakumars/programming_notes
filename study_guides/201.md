...
To review, in the **normal flow**, block boxes are positioned starting in the upper left of their container element and stack from top to bottom. Inline boxes are laid out horizontally, one after the other beginning at the top of the container element. When there isn't enough space to fit an inline box on the current line, it is wrapped to the next line.

### Floats

In the **float** model, a box is first laid out according to the normal flow, then taken out of the flow and shifted to the left or right as far as possible. Surrounding element content is able to flow alongside a float.
