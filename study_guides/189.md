...
**Scalar Subquery:** The nested query returns a single value (e.g. aggregation) that can then be used with a SQL operator in a WHERE clause.

**Column Subquery:** The nested query returns an array of values that can be used with a FROM clause or a combo WHERE/IN clause.
