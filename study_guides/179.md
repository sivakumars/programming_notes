...
Sinatra makes it very simple to write Ruby code that runs when a user visits a particular URL. First we `require` `sinatra` and `sinatra/reloader`, which causes the app to reload its files every time we load a page, making development easier.
