...
OOP addresses this by using containers to bundle related data and the common instructions performed with the data. The container is called an **object** and its attributes (related data) and behavior (common instructions) are outlined in its **class**.

In Ruby, everything is an object. When defining an object in a class, we typically focus on two things: *state* (attributes) and *behavior*. In Ruby, state is implemented using *instance variables* and behavior is implemented using *methods*. Strings, Arrays, Hashes, and Integers are some of the objects built-in to the Ruby language. 
