...
Instance variables (`@`) track information about an object's state. The `Dog` class below has a `name` variable and two instance methods to assign (`set_name`) and retrieve (`get_name`) the `name` of a `Dog` object.  
